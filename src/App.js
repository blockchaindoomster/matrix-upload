 ...
 */
    const onGenerate = async () => {
        ...
        for (let j=0; j< rowNum; j=j+5){
            let matrix = new Array(5).fill(0).map(() => new Array(16).fill(0));
            let indexs = new Array(5).fill(0).map(() => 0);
            let cpus = new Array(5).fill(0).map(() => 0);
        ...
        request({url: '/fetch', method: 'GET'})
            .then(async (response) => {
                ...
                for (let j=0; j< rowNum; j=j+5) {
                    ...
                    const matrix = new Array(5).fill(0).map(() => new Array(16).fill(0));
                    const indexs = new Array(5).fill(0).map(() => 0);
                    const cpus = new Array(5).fill(0).map(() => 0);
        ...
        <div className="App">
            <header className="App-header">
                ...
                <div className="App-button" style={{width: "60%"}}>
                    ...
                    <div>
                        <Link to="/chart" target="_blank" state="123" ><button>view transaction history</button></Link>
                    </div>
                    <div>
                        <a href="https://rinkeby.etherscan.io/address/0x48793d5C8F9497a84C25b7dD46d89f6236802d07" target="_blank" style={{color:'#FFFFFF'}}>view contract</a>
                    </div>
                </div>
